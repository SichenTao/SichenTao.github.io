...
const READER_EXPLICIT_ALL_CATEGORIES = ["field"];
...
function appendSelectionValue(currentValues, value) {
  if (!value || value === "all") return selectionList(currentValues);
  const values = selectionList(currentValues);
  return values.includes(value) ? values : [...values, value];
}

function removeSelectionValue(currentValues, value) {
  return selectionList(currentValues).filter((item) => item !== value);
}

function toggleSelectionValue(currentValues, value) {
  if (!value || value === "all" || value === MULTI_SELECT_ACTIVE_VALUE) {
    return selectionList(currentValues);
  }
  return selectionList(currentValues).includes(value)
    ? removeSelectionValue(currentValues, value)
    : appendSelectionValue(currentValues, value);
}

function previewSelectionValues(currentValues, value) {
  return toggleSelectionValue(currentValues, value);
}

function toggleMenuSelection(currentValues, value, selectionState, category) {
  if (!value || value === MULTI_SELECT_ACTIVE_VALUE) {
    return selectionList(currentValues);
  }
  if (value === "all") {
    setExplicitAllSelection(selectionState, category, true);
    return [];
  }
  const nextValues = toggleSelectionValue(currentValues, value);
  setExplicitAllSelection(selectionState, category, nextValues.length === 0);
  return nextValues;
}

function removeTagSelection(currentValues, value, selectionState, category) {
  if (value === "all") {
    setExplicitAllSelection(selectionState, category, false);
    return selectionList(currentValues);
  }
  const nextValues = removeSelectionValue(currentValues, value);
  setExplicitAllSelection(selectionState, category, nextValues.length === 0);
  return nextValues;
}
...
const state = {
  ...
  researchJcrFilter: [],
  researchCasFilter: [],
  researchCasTopFilter: [],
  researchImpactFilter: [],
  researchAuthorFilter: [],
  ...
  readerQuery: "",
  readerFieldFilters: [],
  readerExplicitAll: createExplicitAllState(READER_EXPLICIT_ALL_CATEGORIES),
...
const UI_TEXT = {
  en: {
    ...
    pageTitle: "学术前沿",
    pageDescription:
      "学术前沿 is an evidence-based research workspace for browsing papers and venue metrics across active research directions.",
    brandTitle: "学术前沿",
    heroEyebrow: "学术前沿",
    heroPrimaryAction: "Open archive",
    heroSecondaryAction: "Review the guide",
    heroHeadline:
      "Record published papers, venue metrics, researchers, and source links in one place.",
    statusCardTitle: "Current focus",
    snapshotLabel: "Latest snapshot",
    nextRunLabel: "Next planned run",
    sourcesCoveredLabel: "Sources",
    activeFocusLabel: "Current focus",
    ...
    trendKicker: "Tracked records",
    trendTitle: "Current tracked records",
    signalRecentPapersTitle: "Latest papers",
    signalArchiveTitle: "PDF archive status",
    signalVenueTitle: "Venue coverage",
    signalResearchersTitle: "Tracked researchers",
    ...
    researchLauncherNote: "Research-planning controls are not published on the public site.",
    ...
    readerSelectedFieldsLabel: "Selected fields",
    ...
    selectedProblemsLabel: "Selected Problems",
    selectedMethodsLabel: "Selected Methods",
    ...
    yearOptionAll: "Year",
    typeOptionAll: "Publication type",
    problemFieldPlaceholder: "Problem field",
    methodFieldPlaceholder: "Method field",
    jcrOptionAll: "JCR quartile",
    casOptionAll: "CAS quartile",
    casTopOptionAll: "Top quartile flag",
    impactOptionAll: "Impact factor range",
    activeFieldTagsLabel: "Selected tags",
    jcrNotListedOption: "JCR not listed",
    casTopOption: "CAS Top",
    casNotListedOption: "CAS not listed",
    impactRange10Plus: "IF >= 10",
    impactRange5To10: "IF 5-10",
    impactRange0To5: "IF 0-5",
    impactNotListedOption: "IF not listed",
    typeOptionJournal: "Journal",
    typeOptionConference: "Conference",
    typeOptionPreprint: "Preprint",
    statusOptionAll: "Submission status",
    statusOptionMustRead: "Tracked",
    statusOptionMonitor: "Monitor",
    statusOptionArchive: "Archive",
    teamOptionAll: "Author",
    venueOptionAll: "Publication venue",
    sortRecent: "Newest first",
    sortVenue: "Venue name",
    sortYear: "Metric year",
    sortCitations: "Citation count",
    sortImpact: "Impact factor high",
    ...
    navGuide: "Workflow",
    whyThisDomainMatters: "Domain record",
    signalsToWatch: "Recorded items",
    whatShouldTriggerActionNext: "Current evidence",
  ...
  zh: {
    htmlLang: "zh-CN",
    pageTitle: "学术前沿",
    pageDescription: "一个用于浏览论文库与出版指标的证据型研究工作区，服务于活跃研究方向的前沿追踪。",
    brandTitle: "学术前沿",
    heroEyebrow: "学术前沿",
    heroPrimaryAction: "打开归档",
    heroSecondaryAction: "查看说明",
    heroHeadline: "把已记录论文、期刊指标、研究者与来源链接集中保存在同一界面中。",
    statusCardTitle: "当前聚焦",
    snapshotLabel: "最新快照",
    nextRunLabel: "下次计划运行",
    sourcesCoveredLabel: "来源",
    activeFocusLabel: "当前聚焦",
    domainsKicker: "方向焦点",
    domainsTitle: "领域记录",
    overviewKicker: "概览",
    overviewTitle: "学术前沿",
    ...
    trendKicker: "跟踪记录",
    trendTitle: "当前跟踪记录",
    signalRecentPapersTitle: "最新论文",
    signalArchiveTitle: "PDF 归档状态",
    signalVenueTitle: "发表渠道覆盖",
    signalResearchersTitle: "已记录研究者",
    ...
    researchLauncherNote: "公开站点不提供研究规划控件。",
    ...
    readerSelectedFieldsLabel: "已选领域",
    ...
    selectedProblemsLabel: "已选问题",
    selectedMethodsLabel: "已选方法",
    ...
    yearOptionAll: "年份",
    typeOptionAll: "发表类型",
    problemFieldPlaceholder: "问题领域",
    methodFieldPlaceholder: "方法领域",
    jcrOptionAll: "JCR 分区",
    casOptionAll: "中科院分区",
    casTopOptionAll: "是否分区 Top",
    impactOptionAll: "影响因子区间",
    activeFieldTagsLabel: "已选标签",
    jcrNotListedOption: "JCR 未公开",
    casTopOption: "中科院 Top",
    casNotListedOption: "中科院未公开",
    impactRange10Plus: "IF >= 10",
    impactRange5To10: "IF 5-10",
    impactRange0To5: "IF 0-5",
    ...
    statusOptionAll: "投稿状态",
    statusOptionMustRead: "已记录",
    ...
    navDownloads: "PDF 队列",
    navGuide: "流程",
    whyThisDomainMatters: "领域记录",
    signalsToWatch: "已记录条目",
    whatShouldTriggerActionNext: "当前证据",
    tensionLabel: "张力：",
    reposLabel: "仓库：",
    papersLabel: "论文：",
    authorsLabel: "作者",
  ...
  ja: {
    ...
    pageTitle: "学術フロンティア",
    pageDescription:
      "論文庫と掲載先指標を閲覧し、アクティブな研究方向を追跡するための証拠ベースの研究ワークスペース。",
    brandTitle: "学術フロンティア",
    heroEyebrow: "学術フロンティア",
    heroPrimaryAction: "アーカイブを開く",
    heroSecondaryAction: "ガイドを見る",
    heroHeadline: "記録済み論文、会場指標、研究者、出典リンクを一つの画面で整理します。",
    ...
    trendKicker: "追跡記録",
    trendTitle: "現在の追跡記録",
    signalRecentPapersTitle: "最新論文",
    signalArchiveTitle: "PDF アーカイブ状態",
    signalVenueTitle: "掲載先カバレッジ",
    signalResearchersTitle: "記録済み研究者",
    ...
    researchLauncherNote: "公開サイトでは研究計画用コントロールを提供しません。",
    ...
    readerSelectedFieldsLabel: "選択中の分野",
    ...
    selectedProblemsLabel: "選択中の問題",
    selectedMethodsLabel: "選択中の手法",
    ...
    yearOptionAll: "年",
    typeOptionAll: "発表種別",
    problemFieldPlaceholder: "問題領域",
    methodFieldPlaceholder: "手法領域",
    jcrOptionAll: "JCR 区分",
    casOptionAll: "CAS 区分",
    casTopOptionAll: "Top 区分",
    impactOptionAll: "IF 範囲",
    activeFieldTagsLabel: "選択中のタグ",
    jcrNotListedOption: "JCR 未公開",
    casTopOption: "CAS Top",
    casNotListedOption: "CAS 未公開",
    impactRange10Plus: "IF >= 10",
    impactRange5To10: "IF 5-10",
    impactRange0To5: "IF 0-5",
    ...
    statusOptionAll: "投稿状態",
    statusOptionMustRead: "記録済み",
    ...
    navGuide: "フロー",
    whyThisDomainMatters: "分野記録",
    signalsToWatch: "記録項目",
    whatShouldTriggerActionNext: "現在の証拠",
    tensionLabel: "緊張点：",
    reposLabel: "リポジトリ：",
    papersLabel: "論文：",
    authorsLabel: "著者",
...
const PATTERN_TRANSLATORS = [
  ...
  {
    regex:
      /^(.*?) at (.*?) currently records (.*?)\. The current feed captures (\d+) recent EC papers since ([0-9-]+)\.(?: Recorded code items include (.*)\.)?$/,
    render: (lang, name, institution, focus, count, date, repoSignal) => {
      if (lang === "zh") {
        return `${name}（${institution}）当前记录方向为 ${localizeText(
          focus
        )}。当前信息流已抓取自 ${date} 以来的 ${count} 篇近期 EC 相关论文。${
          repoSignal ? `当前代码记录包括 ${localizeText(repoSignal)}。` : ""
        }`;
      }

      return `${name}（${institution}）の現在の記録対象は ${localizeText(
        focus
      )}です。現在のフィードでは ${date} 以降の最近の EC 論文を ${count} 件取得しています。${
        repoSignal ? `コード記録として ${localizeText(repoSignal)} も確認できます。` : ""
      }`;
    },
  },
  {
    regex: /^(.*?) has recent records around (.*?); this recent (.*?) paper is recorded\.$/,
    render: (lang, name, topic, venue) =>
      ({
        zh: `${name} 在${localizeText(topic)}方向上有近期记录；已收录一篇近期发表于 ${localizeText(
          venue
        )} 的论文。`,
        ja: `${name} には${localizeText(topic)}周辺の最近の記録があり、${localizeText(
          venue
        )} に掲載された最近の論文を収録しています。`,
      })[lang],
  },
  {
    regex: /^(.*?) remains active in (.*?); this recent (.*?) paper is recorded\.$/,
    render: (lang, name, focus, venue) =>
      ({
        zh: `${name} 在${localizeText(focus)}方面仍有持续记录；已收录一篇近期发表于 ${localizeText(
          venue
        )} 的论文。`,
        ja: `${name} には${localizeText(focus)}に関する継続記録があり、${localizeText(
          venue
        )} の最近の論文を収録しています。`,
      })[lang],
  },
...
function normalizedResearchYearRange(bounds = researchYearBounds()) {
  ...
  return {
    ...
    end: state.researchYearOpenEnded ? maxYear : normalizedEnd,
  };
}

function syncResearchYearState(bounds = researchYearBounds()) {
  const normalized = normalizedResearchYearRange(bounds);
  state.researchYearStart = String(normalized.start);
  state.researchYearEnd = String(normalized.end);
  return normalized;
}

function setResearchYearControlValue(controlId, rawValue) {
  const { minYear, maxYear } = researchYearBounds();
  const parsedValue = Number.parseInt(String(rawValue || ""), 10);
  if (!Number.isFinite(parsedValue)) return false;
  const safeValue = Math.min(Math.max(parsedValue, minYear), maxYear);

  if (controlId === "research-year-start" || controlId === "research-year-start-input") {
    state.researchYearStart = String(safeValue);
  ...
  } else if (controlId === "research-year-end" || controlId === "research-year-end-input") {
    state.researchYearOpenEnded = false;
    state.researchYearEnd = String(safeValue);
  ...
  syncResearchYearState({ minYear, maxYear });
  return true;
}

function updateResearchYearRangePreview(bounds = researchYearBounds()) {
  const normalized = syncResearchYearState(bounds);
  const intervalDisplay = document.querySelector("[data-research-year-interval]");
  const startSlider = byId("research-year-start");
  const endSlider = byId("research-year-end");
  const startInput = byId("research-year-start-input");
  const endInput = byId("research-year-end-input");
  const fill = document.querySelector(".research-range-track-fill");
  const openEndedNote = document.querySelector("[data-research-year-open-note]");

  if (intervalDisplay) {
    intervalDisplay.textContent = researchYearIntervalLabel(state.researchYearStart, state.researchYearEnd, state.researchYearOpenEnded);
  }
  ...
  if (openEndedNote) {
    openEndedNote.textContent = state.researchYearOpenEnded ? ui("researchPresentLabel") : "";
  }
}
...
function syncResearchLauncherState(papers = papersForDomain()) {
  ...
  const casTopBands = papers.some((paper) => paperCasTopValue(paper)) ? ["top"] : [];
  ...
  state.researchAuthorFilter = selectionList(state.researchAuthorFilter).filter((author) => authors.includes(author));
  state.researchJcrFilter = selectionList(state.researchJcrFilter).filter((band) => jcrBands.includes(band));
  state.researchCasFilter = selectionList(state.researchCasFilter).filter((band) => casBands.includes(band));
  state.researchCasTopFilter = selectionList(state.researchCasTopFilter).filter((band) => casTopBands.includes(band));
  state.researchImpactFilter = selectionList(state.researchImpactFilter).filter((band) => impactBands.includes(band));
  ...
  } else if (state.researchImpactFilter.length) {
...
function researchSelectedTagEntries(papers = papersForDomain()) {
  ...
  const entries = [
    ...tagEntriesForSelection("problem", state.researchProblemFieldFilters, state.researchExplicitAll.problem),
    ...tagEntriesForSelection("method", state.researchMethodFieldFilters, state.researchExplicitAll.method),
    ...tagEntriesForSelection("jcr", state.researchJcrFilter, state.researchExplicitAll.jcr),
    ...tagEntriesForSelection("cas", state.researchCasFilter, state.researchExplicitAll.cas),
    ...tagEntriesForSelection("casTop", state.researchCasTopFilter, state.researchExplicitAll.casTop),
    ...tagEntriesForSelection("impact", state.researchImpactFilter, state.researchExplicitAll.impact),
    ...tagEntriesForSelection("author", state.researchAuthorFilter, state.researchExplicitAll.author),
  ];
...
function removeResearchSelectedTag(entry) {
  ...
  if (entry.category === "problem") {
    state.researchProblemFieldFilters = removeTagSelection(state.researchProblemFieldFilters, entry.value, state.researchExplicitAll, "problem");
    return;
  }
  if (entry.category === "method") {
    state.researchMethodFieldFilters = removeTagSelection(state.researchMethodFieldFilters, entry.value, state.researchExplicitAll, "method");
    return;
  }
  if (entry.category === "jcr") {
    state.researchJcrFilter = removeTagSelection(state.researchJcrFilter, entry.value, state.researchExplicitAll, "jcr");
    return;
  }
  if (entry.category === "cas") {
    state.researchCasFilter = removeTagSelection(state.researchCasFilter, entry.value, state.researchExplicitAll, "cas");
    return;
  }
  if (entry.category === "casTop") {
    state.researchCasTopFilter = removeTagSelection(state.researchCasTopFilter, entry.value, state.researchExplicitAll, "casTop");
    return;
  }
  if (entry.category === "impact") {
    state.researchImpactFilter = removeTagSelection(state.researchImpactFilter, entry.value, state.researchExplicitAll, "impact");
    return;
  }
  if (entry.category === "author") {
    state.researchAuthorFilter = removeTagSelection(state.researchAuthorFilter, entry.value, state.researchExplicitAll, "author");
    return;
  }
  if (entry.category === "yearRange") {
...
function filterToolbarLabel() {
  return ui("filtersLabel");
}

function statusPanelTitle() {
  return ui("statusCardTitle");
}

function domainFactSummary(paperCount, teamCount, venueCount) {
  if (state.language === "zh") {
    return `${paperCount} 篇论文 · ${teamCount} 位研究者 · ${venueCount} 个发表渠道`;
  }

  if (state.language === "ja") {
    return `論文 ${paperCount} 件 · 研究者 ${teamCount} 名 · 掲載先 ${venueCount} 件`;
  }

  return `${paperCount} papers · ${teamCount} researchers · ${venueCount} venues`;
}
...
function normalizeQuartileValue(value) {
  const normalized = String(value || "").trim().toUpperCase();
  if (!normalized || normalized === "NONE" || normalized === "N/A" || normalized === "NA" || normalized === "__NOT_LISTED__") {
    return "__not_listed__";
  }
  return normalized;
}

function paperJcrQuartileValue(paper) {
  return normalizeQuartileValue(paper?.metrics?.jcrQuartile);
}

function paperCasQuartileValue(paper) {
  return normalizeQuartileValue(paper?.metrics?.casQuartile);
}

function paperCasTopValue(paper) {
  return Boolean(paper?.metrics?.casTop);
}

function paperImpactBucketValue(paper) {
  const value = paperImpactFactorValue(paper);
...
function paperMatchesJcrFilter(paper, filterValue) {
  if (filterValue === "all") return true;
  if (filterValue === "not-listed") return paperJcrQuartileValue(paper) === "__not_listed__";
  return paperJcrQuartileValue(paper) === String(filterValue || "").toUpperCase();
}
...
function paperMatchesImpactFilter(paper, filterValue) {
  if (filterValue === "all") return true;
  return paperImpactBucketValue(paper) === filterValue;
}

function jcrFilterTagLabel(value) {
  if (value === "not-listed") return ui("jcrNotListedOption");
  return `JCR ${String(value || "").toUpperCase()}`;
}
...
function impactFilterTagLabel(value) {
  if (value === "10+") return ui("impactRange10Plus");
  if (value === "5-10") return ui("impactRange5To10");
  if (value === "0-5") return ui("impactRange0To5");
  if (value === "not-listed") return ui("impactNotListedOption");
  return String(value || "");
}
...
function renderHero() {
  ...
  const domainPapers = papersForDomain();
  const domainTeams = related(data.teams);
  const downloadItems = related(data.downloadQueue);
  const domainVenues = uniqueStrings(domainPapers.map((paper) => localizeText(paper.venue)));
  ...
  setText("heroRole", domain ? domainFactSummary(domainPapers.length, domainTeams.length, domainVenues.length) : "--");
  setRichHtml("heroAffiliation", `${ui("nextRunLabel")}: ${localizeText(data.snapshot.nextRun || "--")}`);
  setRichHtml("headline", ui("heroHeadline"));
  ...
  const cards = [
    {
      ...
      meta: plainTextByLanguage({
        en: "tracked teams and researchers",
        zh: "已记录研究者与团队",
        ja: "記録済み研究者・チーム",
      }),
    },
    {
      ...
      meta: plainTextByLanguage({
        en: "current paper cards in this lane",
        zh: "当前方向下可直接浏览的论文卡片",
        ja: "現在のレーンで確認できる論文カード",
      }),
    },
    {
    ...
    },
    {
      ...
      meta: plainTextByLanguage({
        en: "still waiting for browser-assisted capture",
        zh: "仍需浏览器辅助抓取",
        ja: "ブラウザ補助取得がまだ必要",
      }),
    },
...
function paperMatchesActiveFilters(paper, overrides = {}) {
  const activeYear = selectionList(overrides.year ?? state.yearFilter);
  const activeType = selectionList(overrides.type ?? state.typeFilter);
  const activeProblemTags = selectionList(overrides.problemTags ?? state.problemFieldFilters);
  const activeMethodTags = selectionList(overrides.methodTags ?? state.methodFieldFilters);
  const activeJcr = selectionList(overrides.jcr ?? state.jcrFilter);
  const activeCas = selectionList(overrides.cas ?? state.casFilter);
  const activeCasTop = selectionList(overrides.casTop ?? state.casTopFilter);
  const activeImpact = selectionList(overrides.impact ?? state.impactFilter);
  const activeTeam = selectionList(overrides.team ?? state.teamFilter);
  const activeQuery = overrides.query ?? normalizedPaperQuery();

  const authors = paperAuthorNames(paper);
  const matchesYear = !activeYear.length || activeYear.includes(paperYearValue(paper));
  const matchesType = !activeType.length || activeType.includes(paperTypeValue(paper));
  const matchesProblemTags = !activeProblemTags.length || activeProblemTags.some((tag) => paperProblemFields(paper).includes(tag));
  const matchesMethodTags = !activeMethodTags.length || activeMethodTags.some((tag) => paperMethodFields(paper).includes(tag));
  const matchesJcr = !activeJcr.length || activeJcr.some((filterValue) => paperMatchesJcrFilter(paper, filterValue));
  const matchesCas = !activeCas.length || activeCas.some((filterValue) => paperMatchesCasFilter(paper, filterValue));
  const matchesCasTop = !activeCasTop.length || activeCasTop.some((filterValue) => paperMatchesCasTopFilter(paper, filterValue));
  const matchesImpact = !activeImpact.length || activeImpact.some((filterValue) => paperMatchesImpactFilter(paper, filterValue));
  const matchesTeam = !activeTeam.length || activeTeam.some((team) => authors.includes(team));
...
function selectedTagEntries() {
  return [
    ...tagEntriesForSelection("year", state.yearFilter, state.paperExplicitAll.year),
    ...tagEntriesForSelection("type", state.typeFilter, state.paperExplicitAll.type),
    ...tagEntriesForSelection("problem", state.problemFieldFilters, state.paperExplicitAll.problem),
    ...tagEntriesForSelection("method", state.methodFieldFilters, state.paperExplicitAll.method),
    ...tagEntriesForSelection("jcr", state.jcrFilter, state.paperExplicitAll.jcr),
    ...tagEntriesForSelection("cas", state.casFilter, state.paperExplicitAll.cas),
    ...tagEntriesForSelection("casTop", state.casTopFilter, state.paperExplicitAll.casTop),
    ...tagEntriesForSelection("impact", state.impactFilter, state.paperExplicitAll.impact),
    ...tagEntriesForSelection("team", state.teamFilter, state.paperExplicitAll.team),
  ];
}
...
function normalizePaperFilterState(facets) {
  ...
  state.jcrFilter = selectionList(state.jcrFilter).filter((band) => jcrBands.includes(band));
  state.casFilter = selectionList(state.casFilter).filter((band) => casBands.includes(band));
  state.casTopFilter = selectionList(state.casTopFilter).filter((band) => casTopBands.includes(band));
  state.impactFilter = selectionList(state.impactFilter).filter((band) => impactBands.includes(band));
  state.teamFilter = selectionList(state.teamFilter).filter((team) => authors.includes(team));
  if (state.paperExplicitAll.year) {
    state.yearFilter = [];
  } else if (state.yearFilter.length) {
    setExplicitAllSelection(state.paperExplicitAll, "year", false);
  }
  if (state.paperExplicitAll.type) {
    state.typeFilter = [];
  } else if (state.typeFilter.length) {
  ...
  if (state.paperExplicitAll.jcr) {
    state.jcrFilter = [];
  } else if (state.jcrFilter.length) {
    setExplicitAllSelection(state.paperExplicitAll, "jcr", false);
  }
  if (state.paperExplicitAll.cas) {
    state.casFilter = [];
  } else if (state.casFilter.length) {
    setExplicitAllSelection(state.paperExplicitAll, "cas", false);
  }
  if (state.paperExplicitAll.casTop) {
    state.casTopFilter = [];
  } else if (state.casTopFilter.length) {
    setExplicitAllSelection(state.paperExplicitAll, "casTop", false);
  }
  if (state.paperExplicitAll.impact) {
    state.impactFilter = [];
  } else if (state.impactFilter.length) {
    setExplicitAllSelection(state.paperExplicitAll, "impact", false);
  }
  if (state.paperExplicitAll.team) {
    state.teamFilter = [];
...
function renderMetrics() {
  ...
  const casTopBands = venues.some((venue) => metricVenueMatchesCasTop(venue, ["top"], state.metricYearFilter)) ? ["top"] : [];
  ...
  state.metricCasTopFilter = selectionList(state.metricCasTopFilter).filter((value) => casTopBands.includes(value));
...
function readerFieldFilters() {
  state.readerFieldFilters = selectionList(state.readerFieldFilters);
  return state.readerFieldFilters;
}

function readerControlsDirty() {
  return Boolean(state.readerQuery)
    || readerFieldFilters().length > 0
    || !readerDisplayLanguagesEqual(readerDisplayLanguages(), defaultReaderDisplayLanguages());
}
...
function readerLanguageShortLabel(locale) {
  return {
    en: "EN",
    zh: "中文",
    ja: "日本語",
  }[locale] || LOCALE_CATALOG[locale]?.label || locale;
}

function localizedLabelSeparator() {
    ...
    article.innerHTML = `
      <div class="frontier-reader-card-body">
        ...
        <span class="frontier-reader-dek-group">
          ${authors ? `<span class="frontier-reader-authors">${escapeHtml(ui("authorsLabel"))}${localizedLabelSeparator()}${escapeHtml(authors)}</span>` : ""}
...
function renderFrontierReaderToolbar() {
  ...
  if (reset) {
    ...
    reset.disabled = !readerControlsDirty();
  }
...
function renderReaderResetButtons() {
  document.querySelectorAll("[data-frontier-reader-reset]").forEach((reset) => {
    reset.setAttribute("aria-label", ui("resetLabel"));
    reset.setAttribute("title", ui("resetLabel"));
    reset.disabled = !readerControlsDirty();
  });
...
function renderAcademicPaperArticle() {
  ...
  container.innerHTML = `
    ...
    <header class="fb-article-head">
      ...
      ${article.authors ? `<div class="fb-article-dek-group"><p>${escapeHtml(ui("authorsLabel"))}${localizedLabelSeparator()}${escapeHtml(article.authors)}</p></div>` : ""}
...
function signalArchiveLines() {
  const downloadItems = related(domainData().downloadQueue);
  return [
    `${ui("localPdfsLabel")}: ${papersForDomain().filter((paper) => localArchiveEntry(paper.id)).length}`,
    `${ui("downloadModeDirect")}: ${downloadItems.filter((item) => item.downloadMode === "direct-http").length}`,
    `${ui("downloadModeBrowser")}: ${downloadItems.filter((item) => item.downloadMode === "openclaw-browser").length}`,
    `${ui("downloadModeManual")}: ${downloadItems.filter((item) => item.downloadMode === "manual").length}`,
  ];
}

function signalVenueLines() {
  return uniqueStrings(papersForDomain().map((paper) => localizeText(paper?.venue || ""))).slice(0, 5);
}

function signalResearcherLines() {
  return uniqueStrings(related(domainData().teams).map((team) => String(team?.name || "").trim())).slice(0, 5);
}

function renderSignalRecordCard(title, items) {
  const card = el("article", "trend-card");
  card.appendChild(el("h4", "", title));
  if (!items.length) {
    card.appendChild(el("div", "empty", plainTextByLanguage({
      en: "No items available for this section yet.",
      zh: "这个部分暂时还没有条目。",
      ja: "このセクションにはまだ項目がありません。",
    })));
  ...
  items.forEach((item) => {
    const row = el("li");
    row.innerHTML = `<span class="rich-text">${richTextHtml(item)}</span>`;
...
function renderTrends() {
  renderCardGrid(
    "trendStack",
    [
      { title: ui("signalRecentPapersTitle"), items: signalRecentPaperLines() },
      { title: ui("signalArchiveTitle"), items: signalArchiveLines() },
      { title: ui("signalVenueTitle"), items: signalVenueLines() },
      { title: ui("signalResearchersTitle"), items: signalResearcherLines() },
    ],
    (item) => renderSignalRecordCard(item.title, item.items)
  );
}
...
function renderTeams() {
  renderCardGrid("teamGrid", related(domainData().teams), (team) => {
    ...
    const links = el("div", "link-row");
    const homepageHref = normalizeUrl(team?.links?.homepage);
    const scholarHref = normalizeUrl(team?.links?.googleScholar);
    const openalexHref = normalizeUrl(team?.links?.openalex);
    if (homepageHref) links.appendChild(actionLink(ui("teamHomepageAction"), homepageHref));
    if (scholarHref) links.appendChild(actionLink("Google Scholar", scholarHref));
    if (openalexHref) links.appendChild(actionLink("OpenAlex", openalexHref));
...
function bindControls() {
  ...
  if (!paperSearch) return;

  yearFilter?.addEventListener("change", (event) => {
    const value = event.target.value;
    state.yearFilter = toggleMenuSelection(state.yearFilter, value, state.paperExplicitAll, "year");
  ...
  typeFilter?.addEventListener("change", (event) => {
    const value = event.target.value;
    state.typeFilter = toggleMenuSelection(state.typeFilter, value, state.paperExplicitAll, "type");
    event.target.value = "all";
    renderPaperControls();
    renderPapers();
  });
  problemFilter?.addEventListener("change", (event) => {
    const value = event.target.value;
    state.problemFieldFilters = toggleMenuSelection(state.problemFieldFilters, value, state.paperExplicitAll, "problem");
    event.target.value = "all";
    renderPaperControls();
    renderPapers();
  });
  methodFilter?.addEventListener("change", (event) => {
    const value = event.target.value;
    state.methodFieldFilters = toggleMenuSelection(state.methodFieldFilters, value, state.paperExplicitAll, "method");
    event.target.value = "all";
    renderPaperControls();
    renderPapers();
  });
  jcrFilter?.addEventListener("change", (event) => {
    const value = event.target.value;
    state.jcrFilter = toggleMenuSelection(state.jcrFilter, value, state.paperExplicitAll, "jcr");
    event.target.value = "all";
    renderPaperControls();
    renderPapers();
  });
  casFilter?.addEventListener("change", (event) => {
    const value = event.target.value;
    state.casFilter = toggleMenuSelection(state.casFilter, value, state.paperExplicitAll, "cas");
    event.target.value = "all";
    renderPaperControls();
    renderPapers();
  });
  casTopFilter?.addEventListener("change", (event) => {
    const value = event.target.value;
    state.casTopFilter = toggleMenuSelection(state.casTopFilter, value, state.paperExplicitAll, "casTop");
    event.target.value = "all";
    renderPaperControls();
    renderPapers();
  });
  impactFilter?.addEventListener("change", (event) => {
    const value = event.target.value;
    state.impactFilter = toggleMenuSelection(state.impactFilter, value, state.paperExplicitAll, "impact");
    event.target.value = "all";
    renderPaperControls();
    renderPapers();
  });
  teamFilter?.addEventListener("change", (event) => {
    const value = event.target.value;
    state.teamFilter = toggleMenuSelection(state.teamFilter, value, state.paperExplicitAll, "team");
    event.target.value = "all";
    renderPaperControls();
    renderPapers();
  });
  ...
  activeFieldTags?.addEventListener("click", (event) => {
    ...
    if (category === "year") {
      state.yearFilter = removeTagSelection(state.yearFilter, tag, state.paperExplicitAll, "year");
    } else if (category === "type") {
      state.typeFilter = removeTagSelection(state.typeFilter, tag, state.paperExplicitAll, "type");
    } else if (category === "problem") {
      state.problemFieldFilters = removeTagSelection(state.problemFieldFilters, tag, state.paperExplicitAll, "problem");
    } else if (category === "method") {
      state.methodFieldFilters = removeTagSelection(state.methodFieldFilters, tag, state.paperExplicitAll, "method");
    } else if (category === "jcr") {
      state.jcrFilter = removeTagSelection(state.jcrFilter, tag, state.paperExplicitAll, "jcr");
    } else if (category === "cas") {
      state.casFilter = removeTagSelection(state.casFilter, tag, state.paperExplicitAll, "cas");
    } else if (category === "casTop") {
      state.casTopFilter = removeTagSelection(state.casTopFilter, tag, state.paperExplicitAll, "casTop");
    } else if (category === "impact") {
      state.impactFilter = removeTagSelection(state.impactFilter, tag, state.paperExplicitAll, "impact");
    } else if (category === "team") {
      state.teamFilter = removeTagSelection(state.teamFilter, tag, state.paperExplicitAll, "team");
    }
    renderPaperControls();
    renderPapers();
  });
...
function bindReaderControls() {
  ...
  document.addEventListener("click", (event) => {
    ...
    if (resetButton) {
      state.readerQuery = "";
      state.readerFieldFilters = [];
      resetExplicitAllSelections(state.readerExplicitAll);
    ...
    if (fieldChip) {
      state.readerFieldFilters = toggleMenuSelection(state.readerFieldFilters, fieldChip.dataset.readerFieldChip || "all", state.readerExplicitAll, "field");
      renderFrontierReader();
      return;
    }
...
function bindResearchLauncher() {
  ...
  document.addEventListener("change", (event) => {
    ...
    if (
      target.id === "research-year-start"
      || target.id === "research-year-end"
      || target.id === "research-year-start-input"
      || target.id === "research-year-end-input"
    ) {
      if (!setResearchYearControlValue(target.id, target.value)) {
        updateResearchYearRangePreview();
        return;
      }
      renderDirectionCockpit();
      renderDirectionWorkspace();
    ...
    if (target.id === "research-problem-filter") {
      ...
      state.researchProblemFieldFilters = toggleMenuSelection(state.researchProblemFieldFilters, value, state.researchExplicitAll, "problem");
      target.value = "all";
      renderDirectionCockpit();
      renderDirectionWorkspace();
      renderDomainSwitcher();
    ...
    if (target.id === "research-method-filter") {
      const value = target.value;
      state.researchMethodFieldFilters = toggleMenuSelection(state.researchMethodFieldFilters, value, state.researchExplicitAll, "method");
      target.value = "all";
      renderDirectionCockpit();
      renderDirectionWorkspace();
      renderDomainSwitcher();
    ...
    if (target.id === "research-jcr-filter") {
      const value = target.value;
      state.researchJcrFilter = toggleMenuSelection(state.researchJcrFilter, value, state.researchExplicitAll, "jcr");
      target.value = "all";
    } else if (target.id === "research-cas-filter") {
      const value = target.value;
      state.researchCasFilter = toggleMenuSelection(state.researchCasFilter, value, state.researchExplicitAll, "cas");
      target.value = "all";
    } else if (target.id === "research-cas-top-filter") {
      const value = target.value;
      state.researchCasTopFilter = toggleMenuSelection(state.researchCasTopFilter, value, state.researchExplicitAll, "casTop");
      target.value = "all";
    } else if (target.id === "research-impact-filter") {
      const value = target.value;
      state.researchImpactFilter = toggleMenuSelection(state.researchImpactFilter, value, state.researchExplicitAll, "impact");
      target.value = "all";
    } else if (target.id === "research-author-filter") {
      const value = target.value;
      state.researchAuthorFilter = toggleMenuSelection(state.researchAuthorFilter, value, state.researchExplicitAll, "author");
      target.value = "all";
    } else {
      return;
    }
  ...
  document.addEventListener("input", (event) => {
    ...
    if (target.id === "research-year-start") {
      syncResearchPromptInput();
      if (!setResearchYearControlValue(target.id, target.value)) return;
      updateResearchYearRangePreview();
      return;
    }
...
function renderAll() {
  try {
    ...
    console.error("学术前沿 render failed", error);
