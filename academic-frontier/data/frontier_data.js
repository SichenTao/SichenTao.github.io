window.FRONTIER_DATA = {
  "snapshot": {
    "generatedAt": "2026-04-10 20:08 JST",
    "sourcesCovered": 15,
    "nextRun": {
  ...
  "papers": [
    {
      ...
      "citationCount": 0,
      "abstract": {
        "en": "EvoGO is a fully data-driven evolutionary optimization framework with three stages: data preparation, model training, and population generation. It uses pairwise training data and a generative model to replace traditional reproduction operators with a scalable population-generation mechanism.",
        "ja": "EvoGO は、データ準備、モデル学習、個体群生成の 3 段階で構成される完全データ駆動型の進化最適化フレームワークです。ペアワイズな学習データと生成モデルを用い、従来の再生産演算子をスケーラブルな個体群生成機構へ置き換えます。",
        "zh": "EvoGO 是一个完全数据驱动的进化优化框架，由数据准备、模型训练和种群生成三个阶段组成。它使用成对训练数据与生成模型，用可扩展的种群生成机制替代传统繁殖算子。"
      },
    ...
    },
    {
      ...
      "citationCount": 0,
      "abstract": {
        "en": "The paper presents a cooperative co-evolutionary algorithm for large-scale 0-1 knapsack problems using a core-based grouping strategy. The method decomposes variables while preserving key interactions so search complexity can be reduced in large-scale combinatorial optimization.",
        "ja": "本論文は、コアベースのグルーピング戦略を用いた、大規模 0-1 ナップサック問題向けの協調共進化アルゴリズムを提示します。この手法は、主要な相互作用を保ちながら変数を分解し、大規模組合せ最適化における探索複雑性の低減を図ります。",
        "zh": "本文提出了一种面向大规模 0-1 背包问题的协同协进化算法，采用核心分组策略。该方法在保留关键相互作用的同时对变量进行分解，从而降低大规模组合优化中的搜索复杂度。"
      },
    ...
    },
    {
      ...
      "citationCount": 1,
      "abstract": {
        "en": "EvoRL is an end-to-end GPU-accelerated framework for evolutionary reinforcement learning. It executes environment simulation and evolutionary computation operators on accelerators to support large-population training and scalable experimentation.",
        "ja": "EvoRL は、進化的強化学習のためのエンドツーエンド GPU 加速フレームワークです。環境シミュレーションと進化計算演算子を加速器上で実行し、大規模個体群の学習とスケーラブルな実験を支援します。",
        "zh": "EvoRL 是一个面向进化强化学习的端到端 GPU 加速框架。它在加速器上执行环境模拟与进化计算算子，以支持大规模种群训练和可扩展实验。"
      },
    ...
    },
    {
      ...
      "citationCount": 0,
      "abstract": {
        "en": "TensorNSGA-III preserves the selection and variation logic of NSGA-III while reformulating the algorithm with tensorized GPU-oriented operators. This reformulation accelerates many-objective optimization on accelerators while retaining the core behavior of NSGA-III.",
        "ja": "TensorNSGA-III は、NSGA-III の選択と変異の論理を保ったまま、アルゴリズムを GPU 向けのテンソル演算子へ再構成します。この再構成により、NSGA-III の中核的な挙動を維持しつつ、加速器上での多目的最適化を高速化します。",
        "zh": "TensorNSGA-III 在保留 NSGA-III 选择与变异逻辑的同时，将算法重构为面向 GPU 的张量化算子。该重构在保持 NSGA-III 核心行为的前提下，加速了加速器上的多目标优化计算。"
      },
    ...
    },
    {
      ...
      "citationCount": 1,
      "abstract": {
        "en": "TensorNEAT is a GPU-accelerated library that tensorizes NEAT by reformulating heterogeneous evolving topologies and operations into uniformly shaped tensors. The library supports efficient population-level parallel execution on accelerators and integrates with JAX-based neuroevolution workflows.",
        "ja": "TensorNEAT は、異質な進化トポロジーと演算を均一形状のテンソルへ再定式化することで NEAT をテンソル化する GPU 加速ライブラリです。このライブラリは、加速器上での個体群レベル並列実行を支援し、JAX ベースの神経進化ワークフローと統合されます。",
        "zh": "TensorNEAT 是一个通过将异质演化拓扑与操作重构为统一形状张量来实现 NEAT 张量化的 GPU 加速库。该库支持在加速器上进行种群级高效并行执行，并可与基于 JAX 的神经进化工作流集成。"
      },
    ...
    },
    {
      ...
      "abstract": {
        "en": "The paper presents a GPU-accelerated tree-based genetic programming framework using tensorized tree representations, adaptive multi-level parallelism, and custom CUDA integration. The framework targets efficient population-level parallel execution despite structural heterogeneity among programs.",
        "ja": "本論文は、テンソル化された木表現、適応的な多段並列性、独自の CUDA 統合を用いた GPU 加速型の木ベース遺伝的プログラミングフレームワークを提示します。構造的に不均一なプログラム群に対して、個体群レベルの効率的な並列実行を目標としています。",
        "zh": "本文提出了一个面向 GPU 加速的树式遗传程序框架，采用张量化树表示、自适应多层并行以及定制 CUDA 集成。该框架旨在在程序结构高度异质的情况下实现种群级高效并行执行。"
      },
      "alternateVersions": [
        {
          "type": "preprint",
    ...
    },
    {
      ...
      "abstract": {
        "en": "The paper studies how tensorization can restructure evolutionary multiobjective optimization for GPU execution. It focuses on mapping EMO data structures and operators onto accelerator-friendly representations so many-objective optimization can scale on modern hardware.",
        "ja": "本論文は、テンソル化によって進化的多目的最適化を GPU 実行向けに再構成する方法を検討します。EMO のデータ構造と演算子を加速器向け表現へ写像し、多目的最適化を現代ハードウェア上でスケールさせることに焦点を当てています。",
        "zh": "本文研究如何通过张量化将进化多目标优化重构为适合 GPU 执行的形式。其重点是把 EMO 的数据结构与操作算子映射到对加速器友好的表示上，从而使多目标优化能够在现代硬件上实现扩展。"
      },
      "alternateVersions": [
        {
          "type": "preprint",
    ...
    },
    {
      ...
      "abstract": {
        "en": "MetaDE uses differential evolution to optimize the configuration of differential evolution itself, including strategy and hyperparameter choices. The method treats algorithm configuration as an optimization target rather than relying only on manual tuning or lightweight adaptive heuristics.",
        "ja": "MetaDE は、戦略やハイパーパラメータ選択を含む差分進化の設定そのものを、差分進化によって最適化します。手動調整や軽量な適応ヒューリスティクスだけに依存せず、アルゴリズム設定自体を最適化対象として扱う手法です。",
        "zh": "MetaDE 使用差分进化来优化差分进化自身的配置，包括策略与超参数选择。该方法把算法配置本身视为优化目标，而不是只依赖人工调参或轻量级自适应启发式。"
      },
      "alternateVersions": [
        {
          "type": "preprint",
    ...
    },
    {
      ...
      "abstract": {
        "en": "The paper introduces an evolutionary NAS framework for monocular depth estimation that uses hierarchical parameter mapping to avoid costly per-candidate pretraining. The framework applies evolutionary search to architecture design for monocular depth estimation while reducing the computational cost of candidate evaluation.",
        "ja": "本論文は、単眼深度推定のための進化的 NAS 枠組みを提案し、階層的パラメータマッピングによって候補ごとの高価な事前学習を回避します。この枠組みは、単眼深度推定のアーキテクチャ設計へ進化探索を適用しつつ、候補評価の計算コストを低減します。",
        "zh": "本文提出了一种面向单目深度估计的进化 NAS 框架，利用层级参数映射避免对每个候选架构进行高成本预训练。该框架将进化搜索用于单目深度估计的架构设计，并降低了候选评估的计算开销。"
      },
    ...
    },
    {
      ...
      "abstract": {
        "en": "EvoX presents a distributed GPU-accelerated framework for scalable evolutionary computation in data-intensive and large-scale settings. The framework provides reusable system support for modern EC workloads, including distributed execution and accelerator-oriented computation.",
        "ja": "EvoX は、データ集約的かつ大規模な設定におけるスケーラブルな進化計算のための分散 GPU 加速フレームワークを提示します。このフレームワークは、分散実行と加速器指向計算を含む、現代的な EC ワークロード向けの再利用可能なシステム基盤を提供します。",
        "zh": "EvoX 提出了一种面向数据密集和大规模场景的分布式 GPU 加速进化计算框架。该框架为现代 EC 工作负载提供可复用的系统支持，包括分布式执行与面向加速器的计算机制。"
      },
      "alternateVersions": [
        {
          "type": "preprint",
    ...
    },
    {
      ...
      "abstract": {
        "en": "The survey organizes the use of large language models in evolutionary optimization into modeling and solving stages, and further divides solving into stand-alone optimizers, low-level algorithmic components, and high-level managerial roles. It provides a structured taxonomy of how large language models are being integrated into evolutionary optimization workflows.",
        "ja": "このサーベイは、進化最適化における大規模言語モデルの利用を、モデリング段階と解法段階に整理し、さらに解法段階を単独オプティマイザ、低レベルアルゴリズム部品、高レベル管理役割へと分けています。大規模言語モデルが進化最適化ワークフローへどのように組み込まれているかについて、構造化された分類を提示します。",
        "zh": "该综述将大语言模型在进化优化中的应用划分为建模阶段与求解阶段，并进一步将求解分为独立优化器、底层算法组件和高层管理角色。综述给出了大语言模型融入进化优化工作流的结构化分类。"
      },
      "sourceLineage": {
        "homepage": "Not listed on the selected-publications page as of 2026-04-08.",
        "dblp": "https://dblp.org/pid/18/4198-4.xml",
